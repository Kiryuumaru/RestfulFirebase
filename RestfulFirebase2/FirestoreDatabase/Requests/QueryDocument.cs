        ...
        where TRequest : TransactionRequest
    {
            ...
            foreach (var filter in request.Where.FilterQuery)
            {
                switch (filter)
                {
                    ...
                    case FieldFilterQuery fieldFilter:
                        ...
                        ModelBuilderHelpers.BuildUtf8JsonWriterObject(config, writer, fieldFilter.Value?.GetType(), fieldFilter.Value, jsonSerializerOptions, null, null);
                        writer.WriteEndObject();
                        writer.WriteEndObject();
        ...
        if (request.OrderBy != null)
        {
            ...
            foreach (var orderBy in request.OrderBy.OrderByQuery)
            {
                ...
                writer.WriteStringValue(orderBy.OrderDirection == Direction.Ascending ? "ASCENDING" : "DESCENDING");
                writer.WriteEndObject();
            }
            writer.WriteEndArray();
        }
        if (request.StartAt != null)
        {
            ...
            {
                ModelBuilderHelpers.BuildUtf8JsonWriterObject(config, writer, cursor.Value?.GetType(), cursor.Value, jsonSerializerOptions, null, null);
            }
            writer.WriteEndArray();
        ...
        {
            List<(string field, Direction direction)> orderBy = new();
            List<object> startCursors = new();
            List<object> endCursors = new();

            bool hasDocumentName = false;
            foreach (var query in request.Order.OrderQuery)
            {
                string fieldName = fieldNameFactory(query.PropertyName);

                if (fieldName == DocumentFieldHelpers.DocumentName)
                {
                    if (hasDocumentName)
                    {
                        throw new ArgumentException($"\"{nameof(request.Order)}\" must only contain a single document name.");
                    }
                    hasDocumentName = true;
                }
                else if (hasDocumentName)
                {
                    throw new ArgumentException($"\"{nameof(request.Order)}\" with document name must be place on the end of the query.");
                }

                orderBy.Add((fieldName, query.Direction));
                startCursors.Add(query.StartPosition ?? DocumentFieldHelpers.GetLimitValue(, query.Direction == Direction.Ascending));
                startCursors.Add(query.EndPosition ?? DocumentFieldHelpers.GetLimitValue(, query.Direction == Direction.Ascending));

            }

            writer.WritePropertyName("orderBy");
            writer.WriteStartArray();
            foreach (var (field, direction) in orderBy)
            {
                writer.WriteStartObject();
                writer.WritePropertyName("field");
                writer.WriteStartObject();
            ...
            {
                ModelBuilderHelpers.BuildUtf8JsonWriterObject(config, writer, cursor?.GetType(), cursor, jsonSerializerOptions, null, null);
            }
            writer.WriteEndArray();
            writer.WritePropertyName("before");
            writer.WriteBooleanValue(request.Order.IsStartAfter);
            writer.WriteEndObject();
            ...
            writer.WriteEndArray();
            writer.WritePropertyName("before");
            writer.WriteBooleanValue(!request.Order.IsEndBefore);
            writer.WriteEndObject();
        }
    ...
    where TRequest : TransactionRequest
{
    ...
    /// Gets or sets the order to apply to the query results.
    /// </summary>
    public OrderQuery.Builder? Order { get; set; }
    ...
    where T : class
{
    ...
    internal override async Task<TransactionResponse<QueryDocumentRequest<T>, QueryDocumentResult<T>>> Execute()
    {
        ...
        var firstResponse = await ExecutePage(ActualSkipPage, From, firstFromQuery, objType, propertyInfos, fieldInfos, includeOnlyWithAttribute, Config, jsonSerializerOptions, default);
        if (firstResponse.IsError)
        {
            return new(this, null, firstResponse.Error);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        response = new(
            ...
            new(foundDocuments,
                ...
                (pageNum, ct) =>
                {
                    return ExecutePage(pageNum, fromQuery, firstFromQuery, objType, propertyInfos, fieldInfos, includeOnlyWithAttribute, config, jsonSerializerOptions, ct);
                }),
            null);
        return response;
    }
