...
public partial class RealtimeInstance : ICloneable
{
    ...
    {
        App = app;
        RealtimeDatabase = query.RealtimeDatabase;
        Query = query;
        ...
        where TModel : notnull
    {
        ...
        void RealtimeInstance_Disposing(object sender, EventArgs e)
        {
            if (!realtimeModel.IsDisposedOrDisposing)
            {
                realtimeModel.Dispose();
            }
        }

        Disposing += RealtimeInstance_Disposing;
    ...
    public RealtimeInstance Child(params string[] path)
    {
        ...
        FirebasePathUtilities.EnsureValidAndNonEmptyPath(path);
    ...
    private void DBPut(string? blob, string[] absolutePath)
    {
        ...
        if (Started)
        {
            App.RealtimeDatabase.DBPut(Query.RealtimeDatabase, absolutePath, blob, (Action<(RealtimeDatabaseApp.WriteTask writeTask, RetryExceptionEventArgs err)>)(args =>
            {
                if (IsDisposed)
                {
                    return;
                ...
                {
                    args.err.Retry = writeTaskErrorControl.SendAsync((Func<Task<bool>>)async delegate
                    {
            ...
            {
                writeTaskErrorControl.ConcurrentTokenCount = App.Config.RealtimeDatabaseMaxConcurrentSyncWrites;
            });
        }
    }
    ...
    private string[] DBGetAbsoluteDataPath(string[]? path)
    {
        ...
        absoluteDataPath[0] = RealtimeDatabaseApp.OfflineDatabaseIndicator;
