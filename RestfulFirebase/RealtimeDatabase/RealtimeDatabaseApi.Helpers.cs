...
public partial class RealtimeDatabaseApi
{
    ...
    internal async Task<HttpResponse<T>> ExecuteGet<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(string url, JsonSerializerOptions jsonSerializerOptions, CancellationToken cancellationToken)
    {
    ...
    [UnconditionalSuppressMessage("Trimming", "IL2026:Members annotated with 'RequiresUnreferencedCodeAttribute' require dynamic access otherwise can break functionality when trimming application code", Justification = "<Pending>")]
    internal async Task<HttpResponse<T>> ExecutePut<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T>(MemoryStream stream, string url, JsonSerializerOptions jsonSerializerOptions, CancellationToken cancellationToken)
    {
        HttpResponse<T> response = new();
