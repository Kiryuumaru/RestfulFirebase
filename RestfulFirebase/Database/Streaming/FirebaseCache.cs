...
namespace RestfulFirebase.Database.Streaming
{
    ...
    public class FirebaseCache<T> : IEnumerable<StreamObject<T>>
    {
        ...
        public IEnumerable<StreamObject<T>> PushData(string path, string data, bool removeEmptyEntries = true)
        {
            ...
            if (string.IsNullOrWhiteSpace(data) || data == "null")
            {
                ...
                yield return new StreamObject<T>(key, target);
                yield break;
            }
            ...
            if (obj is IDictionary && !this.isDictionaryType)
            {
                ...
                var objectCollection = new List<StreamObject<object>>();
                var dictionaryType = typeof(Dictionary<,>).MakeGenericType(typeof(string), valueType);
                IDictionary dictionaryInstance;

                if (data.StartsWith("["))
                {
                    var listType = typeof(List<>).MakeGenericType(valueType);
                    var list = JsonConvert.DeserializeObject(data, listType) as IList;
                    dictionaryInstance = Activator.CreateInstance(dictionaryType) as IDictionary;
                    var index = 0;
                    foreach (var item in list) dictionaryInstance.Add(index++.ToString(), item);
                }
                else
                {
                    dictionaryInstance = JsonConvert.DeserializeObject(data, dictionaryType) as IDictionary;
                }

                if (dictionaryInstance != null)
                {
                    foreach (DictionaryEntry item in dictionaryInstance)
                    {
                        objectCollection.Add(new StreamObject<object>((string)item.Key, item.Value));
                    }
                }
                ...
                foreach (var item in objectCollection)
                {
                    ...
                    if (!pathElements.Any())
                    {
                        yield return new StreamObject<T>(item.Key, (T)item.Object);
                    }
                }
                ...
                if (pathElements.Any())
                {
                    ...
                    yield return new StreamObject<T>(pathElements[0], this.dictionary[pathElements[0]]);
                }
            }
            else
            {
        ...
        public IEnumerator<StreamObject<T>> GetEnumerator()
        {
            return this.dictionary.Select(p => new StreamObject<T>(p.Key, p.Value)).GetEnumerator();
        }
